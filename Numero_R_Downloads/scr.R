# ============================================================== #
# ANÁLISE DE DOWNLOADS DO R NO CRAN (2012-2025)
# ============================================================== 3
# Objetivo: Visualizar a evolução temporal dos downloads da linguagem R
#           a partir dos registros históricos do CRAN
# Projeto: Processamento e Análise de Dados Ambientais com R (@proamb.r)
# Autoria: Nícolas Chenquel Nogueira
# Orientação: Profa. Dra. Eliane Barbosa Santos (LAMET/UENF)
# Inspiração Visual: Cédric Scherer (@Z3tt)
# Instituições:
#   - Laboratório de Meteorologia (UENF)
#   - Programa de Extensão Universidade Aberta (PROEx/UENF)
# Dados: http://cran-logs.rstudio.com/ (2012-10-01 a 2025-04-29)
# Metodologia:
#   - Coleta automatizada de logs históricos
#   - Processamento paralelo com future.apply
#   - Visualização com ggsankey + customizações
# Destaques Técnicos:
#   - Arquitetura de dados massivos (≈12 anos diários)
#   - Técnicas de paralelização (multicore)
#   - Sistema de anotações dinâmicas
...
# utilizá-los para produir um gráfico com o número de downloads. Este script é
# original, mas o gráfico foi inspirado na ilustração de Cédric Scherer.

# ATENÇÃO! Dependendo da capacidade do computador, o processo de download e 
# processamento dos dados pode demorar um tempo para ser concluído. Em uma máquina 
# Intel Core i7-7700HQ - RAM 16GB, por exemplo, o processo é concluído em torno de 2 hora.
...
## Iniciando o processo de download e processamento dos dados em paralelo para acelerar o script.

## Preparação, definindo o numero de workers com base no número de núcleos do processador da máquina:
...
## Usando future_lapply para baixar os arquivos em paralelo:
future_lapply(
  X = seq_along(urls),
  FUN = function(i) {
    tryCatch({
...
## Nesta etapa, vamos abrir os arquivos .gz individualmente, ler seus conteúdos e,
## em seguida, escrever um csv na pasta data_raw.
plan(multisession, workers = parallel::detectCores())
...
## Vamos usar novamente o future_lapply() para acelerar o script:
future_lapply(
  X = ls_arquivos,
  FUN = function(x){
    tryCatch({
...
## Agora vamos estudar os dados que coletamos. Podemos perceber que houve
...
## coluna chamada V.x que receba apenas o primeiro caracter da coluna "version".
...
pl <-
  ...
  ## Optei por fazer assim devido ao recuo dos primeiros anos
  ## no grid.
  geom_segment(
    data = ldates,
    mapping =
  ...
  geom_textbox(
    mapping = aes(
      ...
      halign = 0,
      fill = NA,
      label = "Os registros de downloads do CRAN (Comprehensive R Archive Network) foram coletados por métodos automatizados e utilizados para calcular o total mensal de downloads. Apenas as versões principais do R foram consideradas. Ressaltamos que o número de downloads não representa um quantitativo de usuários ativos diretamente, mas pode ser um bom indicativo do crescimento da linguagem."
    ),
    size = 25,
    inherit.aes = F,
    lineheight = 0.2,
